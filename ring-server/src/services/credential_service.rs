...
impl CredentialService {
    ...
    pub fn encrypt(&self, plaintext: &str) -> Result<String> {
        ...
        let nonce = Nonce::from_slice(b"ring-12b-nonce");
...
mod tests {
    ...
    fn test_key() -> [u8; 32] {
        CredentialService::derive_key_from_password("test-password")
    }

    #[test]
    fn encrypt_decrypt_roundtrip() {
        let svc = CredentialService::new(test_key());
        let encrypted = svc.encrypt("hello world").unwrap();
    ...
    fn decrypt_wrong_key_fails() {
        let key_a = CredentialService::derive_key_from_password("password-a");
        let key_b = CredentialService::derive_key_from_password("password-b");
        let svc_a = CredentialService::new(key_a);
        let svc_b = CredentialService::new(key_b);
        let encrypted = svc_a.encrypt("secret").unwrap();
        let result = svc_b.decrypt(&encrypted);
