    ...
    let mut rx = {
        ...
        llm.chat_stream_with_tools(
            ...
            move |name: String, args: serde_json::Value| {
                ...
                Box::pin(async move {
                    crate::services::chat::execute_group_tool(&state, Some(&rid), &user, name, args)
                        .await
                })
            },
            move |full_content: String, token_usage: Option<String>| {
                ...
                Box::pin(async move {
                    ...
                    let ring_name_search = crate::services::search::get_ring_name(&pool, &ring_id)
                        .await
                        .unwrap_or_default();
                    ...
                    tokio::spawn(async move {
                        if let Err(e) =
                            crate::services::group_doc_maintenance::update_active_context(
                                &state_doc,
                                &ring_id_doc,
                                &user_id_doc,
                                &user_row_doc,
                    ...
                    let backend_auto = match crate::services::archive_service::get_backend(
                        &state.db,
                        &ring_id_auto,
                        Some(&user_row),
                        Some(&state.encryption),
                    ...
                    tokio::spawn(async move {
                        let auto_archive: bool =
                            sqlx::query_scalar("SELECT auto_archive FROM rings WHERE id = ?1")
                                .bind(&ring_id_auto)
                                .fetch_one(&pool_auto)
                                .await
                                .unwrap_or(false);
